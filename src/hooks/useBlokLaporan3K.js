...
const KOLEKSI = 'blokLaporan3K'

export function useBlokLaporan3K() {
...
export async function tambahBlok(nama, adaDisiplin, turutan) {
...
export async function kemaskiniBlok(id, data) {
  if (!isFirebaseConfigured) throw new Error('Firebase belum disetup')
  await updateDoc(doc(db, KOLEKSI, id), { ...data, updatedAt: serverTimestamp() })
}

export async function padamBlok(id) {
  if (!isFirebaseConfigured) throw new Error('Firebase belum disetup')
