...
const KOLEKSI = 'rpiPrasidang'

export function useRPIPrasidangList() {
...
export async function tambahPrasidang(data, uid) {
...
export async function kemaskiniPrasidang(id, data, uid) {
  if (!isFirebaseConfigured) throw new Error('Firebase belum disetup')
  await updateDoc(doc(db, KOLEKSI, id), { ...data, updatedAt: serverTimestamp(), updatedBy: uid })
}

export async function padamPrasidang(id) {
  if (!isFirebaseConfigured) throw new Error('Firebase belum disetup')
