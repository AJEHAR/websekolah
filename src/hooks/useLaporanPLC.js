...
const KOLEKSI = 'laporanPLC'

export function useLaporanPLC() {
...
export async function tambahLaporanPLC(data, uid) {
...
export async function kemaskiniLaporanPLC(id, data, uid) {
  if (!isFirebaseConfigured) throw new Error('Firebase belum disetup')
  await updateDoc(doc(db, KOLEKSI, id), { ...data, updatedAt: serverTimestamp(), updatedBy: uid })
}

export async function padamLaporanPLC(id) {
  if (!isFirebaseConfigured) throw new Error('Firebase belum disetup')
