...
import { X, Plus, PenLine, Move, Printer } from 'lucide-react'
...
// Borang SATU Laporan Aktiviti Perjumpaan - dipautkan ikut (tahunSesi,
// unitId, perjumpaan) yang SAMA dengan Perancangan & Kehadiran UBKS -
// Tarikh & Bil. Ahli Hadir auto-cadang dari dua sumber tu bila borang
// dibuka (staff tetap boleh edit semua).
export default function LaporanUBKSForm({ open, unit, perjumpaan, user, onClose, onSelesai, onCetak }) {
  ...
  const [tunjukTtd, setTunjukTtd] = useState(null) // 'setiausaha' | 'guru' | 'gpk'

  useEffect(() => {
    if (!open || !unit || !perjumpaan) return
    let batal = false
    setMemuatkan(true)
    ;(async () => {
        ...
        {memuatkan ? (
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-3 pt-2 border-t border-border">
              ...
              <button onClick={() => onCetak(data)} className="h-12 px-4 rounded-card border border-border text-sm font-medium text-ink flex items-center gap-1.5">
                <Printer size={15} /> Cetak
              </button>
              <button onClick={onClose} className="h-12 px-4 rounded-card border border-border text-sm font-medium text-ink">Tutup</button>
            </div>
          </div>
        )}
      </div>
