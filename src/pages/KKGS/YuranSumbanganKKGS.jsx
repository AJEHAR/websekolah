...
// Papan Pembayaran - SATU papan sama untuk SEMUA orang (nama diambil
// terus dari Senarai Ahli, TIADA padanan emel/akaun langsung). Staff
// biasa - LIHAT sahaja (telus, macam Kewangan). Admin 'kkgs' - urus
// penuh (rekod bayaran, tetapan, resit).
export default function YuranSumbanganKKGS() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs text-inkmuted">Yuran {tahun}: RM{tetapan.yuranBulanan}/bulan × {tetapan.bilanganBulan} bulan (Jan-{NAMA_BULAN[tetapan.bilanganBulan - 1]}).{!bolehUrus && ' Anda boleh LIHAT sahaja.'}</p>
        {bolehUrus && (
                      ...
                      {rekodAhli.map((y) => (
                        <div key={y.id} className="flex items-center justify-between text-xs">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            ...
                            {bolehUrus && <button onClick={() => padamBayaran(y.id)} aria-label="Padam" className="text-brand-red"><Trash2 size={13} /></button>}
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      {bolehUrus && (
        <>
          ...
          {ahliBayar && <ModalBayar key={ahliBayar.id} open onTutup={() => setAhliBayar(null)} ahli={ahliBayar} tarikCadangan={tetapan.yuranBulanan} onSimpan={bayar} />}
        </>
      )}
