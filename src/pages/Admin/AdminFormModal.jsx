...
export default function AdminFormModal({ open, onClose, profile, onSimpan }) {
  if (!open) return null
      ...
      <div className="bg-surface rounded-t-2xl sm:rounded-card w-full sm:max-w-lg max-h-[88vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-ink">
            {profile ? 'Kemas Kini Staff' : 'Tambah Staff Baru'}
          </h2>
          <button onClick={onClose} aria-label="Tutup borang" className="p-1.5 rounded-card hover:bg-base text-inkmuted">
            <X size={18} />
          </button>
        </div>
        <AdminProfileForm profile={profile} onSimpan={onSimpan} onBatal={onClose} />
      </div>
    </div>
  )
}
