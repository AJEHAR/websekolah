...
// data: { tahun, tapisKelas, pelajar, jumlahBulanKeseluruhan, jumlahBesar }
// Ringkasan jumlah HARI HADIR setiap murid ikut bulan (bukan tick harian) -
// 1 muka surat sahaja (bukan pecah ikut bulan macam CetakKehadiranBulanan).
export default function CetakKehadiranTahunan({ data }) {
  const bulanSenarai = Array.from({ length: 12 }, (_, i) => i + 1)
  return (
    <PrintArea>
      <div className="cetak-landskap p-8 text-black">
        <KepalaSuratCetak tajukLaporan={`Jumlah Kehadiran Tahunan — ${data.tahun} — ${data.tapisKelas || 'Semua Kelas'}`} />
